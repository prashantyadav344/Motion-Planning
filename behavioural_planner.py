...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        if self._state == FOLLOW_LANE:
            ...
            # ------------------------------------------------------------------
            if stop_sign_found:
            	print('State update: DECELERATE_TO_STOP')
        ...
        # state.
            ...
            if(closed_loop_speed < STOP_THRESHOLD):
            	print('State update: STAY_STOPPED')
            	closed_loop_speed = 0
        ...
        # the stop sign and transition to the next state.
            ...
            if self._stop_count == STOP_COUNTS:
                ...
                # self._goal_index = ... 
                # self._goal_state = ... 
                while(stop_sign_found) :
                	closest_index += 2
                	_, stop_sign_found = self.check_for_stop_signs(waypoints, closest_index, goal_index+2)
                # --------------------------------------------------------------
                ...
                if not stop_sign_found:
                	print('State update: FOLLOW_LANE')
                	self._stop_count = 0
                	self._state = FOLLOW_LANE
                #   ...
                # --------------------------------------------------------------
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
    	diff_distance = np.linalg.norm((waypoints[i][0] - ego_state[0], waypoints[i][1] - ego_state[1]))
    	if(diff_distance < closest_len):
    		closest_len = diff_distance
    		closest_index = i
