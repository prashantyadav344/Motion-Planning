...
class CollisionChecker:
    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # --------------------------------------------------------------
                print("Circle Offsets:", self._circle_offsets)
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        print('Outside for loop Collison, len of paths:', len(paths))
        for i in range(len(paths)):
            # Handle the case of collision-free paths.
            print('Inside for loop Collison, len of paths:', len(paths))
            if collision_check_array[i]:
                ...
                #score = 
                path = paths[i]
                score = np.linalg.norm((path[0][len(path[0])-1] - goal_state[0],
                	                   path[1][len(path[1])-1] - goal_state[1]))
                print('Path:',i)
                print('Score:', score)
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            ...
                            path1=paths[i]
                            path2=paths[j]
                            proxy = np.linalg.norm((path1[0][len(path1[0])-1]-path2[0][len(path2[0])-1] ,
                            	                   path1[1][len(path1[1])-1]-path2[1][len(path2[1])-1]))
                            print('Proxy:', proxy)
                            score += self._weight * proxy
                            # --------------------------------------------------
