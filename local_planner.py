...
class LocalPlanner:
    def __init__(self, num_paths, path_offset, circle_offsets, circle_radii, 
        ...
        #self._prev_best_path = self._prev_best_path = np.asarray([np.zeros(100), np.zeros(100), np.zeros(100)])
    ...
    def get_goal_state_set(self, goal_index, goal_state, waypoints, ego_state):
        ...
        for i in range(self._num_paths):
            ...
            print('Offset X:', x_offset, 'Y:', y_offset)
            # ------------------------------------------------------------------
    ...
    def plan_paths(self, goal_state_set):
        ...
        for goal_state in goal_state_set:
            path = self._path_optimizer.optimize_spiral(goal_state[0], 
                                                        goal_state[1], 
                                                        goal_state[2])
            print('Current goal:', goal_state)
            #print('Goal state set:', goal_state_set)
            final_dis = np.linalg.norm([path[0][-1] - goal_state[0], 
                                        path[1][-1] - goal_state[1], 
                                        path[2][-1] - goal_state[2]]) 
            print('Final Distance from goal:', final_dis)
            if final_dis > 0.1:
