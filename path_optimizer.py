...
class PathOptimizer:
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        # ...
        term1 = np.multiply(a,s)
        term2 = np.multiply(np.multiply(0.5,b),np.multiply(s,s))
        term3 = np.multiply(np.multiply((1/3),c),np.multiply(s,np.multiply(s,s)))
        term4 = np.multiply(np.multiply(0.25,d),np.multiply(s,np.multiply(s,np.multiply(s,s))))
        thetas = np.add(np.add(term1,term2),np.add(term3,term4))
        #thetas = a*s + (b/2.0)*s**2 + (c/3.0)*s**3 + (d/4.0)*s**4
        #kappa = np.array([a, b, c, d])
        
        #s_1 = np.power(s, 1)/1
        #s_2 = np.power(s, 2)/2
        #s_3 = np.power(s, 3)/3
        #s_4 = np.power(s, 4)/4
        #S   =  np.vstack((s_1, s_2, s_3, s_4))       
        #thetas = np.sum(kappa.T * S.T,axis=1)
